...
if __name__ == "__main__":
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        filename = path.split("/")[-1].split(".")[0].replace("\\", "/")
        out_dir = "output" if opt.out is None else opt.out
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        file_name = f"{out_dir}/{filename.split('/')[-1]}.png"
